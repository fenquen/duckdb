...
namespace duckdb {
    ...
    bool Transformer::TransformParseTree(duckdb_libpgquery::PGList *tree,
        ...
        for (auto entry = tree->head; entry != nullptr; entry = entry->next) {
            ...
            auto sqlStatement = TransformStatement((duckdb_libpgquery::PGNode *) entry->data.ptr_value);
            D_ASSERT(sqlStatement);

            sqlStatement->n_param = ParamCount();
            sqlStatementVector.push_back(move(sqlStatement));
        }
        return true;
    }
