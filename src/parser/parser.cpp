...
namespace duckdb {
    ...
    void Parser::ParseQuery(const string &querySql) {
...

        {
            ...
            postgresParser.Parse(querySql);

            if (!postgresParser.success) {
                if (options.extensions) {
                    for (auto &ext: *options.extensions) {

                        D_ASSERT(ext.parse_function);

                        auto result = ext.parse_function(ext.parser_info.get(), querySql);

                        if (result.type == ParserExtensionResultType::PARSE_SUCCESSFUL) {
                            auto statement = make_unique<ExtensionStatement>(ext, move(result.parse_data));
                            statement->stmt_length = querySql.size();
                ...
                throw ParserException(QueryErrorContext::Format(querySql,
                                                                postgresParser.error_message,
                                                                postgresParser.error_location - 1));
            }

            // empty statement
            if (!postgresParser.parse_tree) {
                return;
            }

            // transform the Postgres parse tree into a list of SQLStatement
            transformer.TransformParseTree(postgresParser.parse_tree, statements);
        }
        ...
        last_statement->stmt_length = querySql.size() - last_statement->stmt_location;
        for (auto &statement: statements) {
            statement->query = querySql;
            if (statement->type == StatementType::CREATE_STATEMENT) {
                auto &create = (CreateStatement &) *statement;
                create.info->sql = querySql.substr(statement->stmt_location, statement->stmt_length);
            }
        }
    }
