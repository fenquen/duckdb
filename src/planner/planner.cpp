...
namespace duckdb {
    ...
    Planner::Planner(ClientContext &clientContext) : binder(Binder::CreateBinder(clientContext)),
    ...
    void Planner::CreatePlan(SQLStatement &sqlStatement) {
        auto &profiler = QueryProfiler::Get(context);
        auto parameter_count = sqlStatement.n_param;
        ...
        try {
            ...
            auto boundStatement = binder->Bind(sqlStatement);

            profiler.EndPhase();

            names = boundStatement.names;
            types = boundStatement.types;

            plan = std::move(boundStatement.plan);

            auto maxTreeDepth = ClientConfig::GetConfig(context).max_expression_depth;

            CheckTreeDepth(*plan, maxTreeDepth);
            ...
            for (auto &extension_op: config.operator_extensions) {
                auto bound_statement = extension_op->Bind(context, *this->binder, extension_op->operator_info.get(), sqlStatement);
    ...
    void Planner::CreatePlan(unique_ptr<SQLStatement> sqlStatement) {
        D_ASSERT(sqlStatement);
        switch (sqlStatement->type) {
            ...
            case StatementType::LOGICAL_PLAN_STATEMENT:
                CreatePlan(*sqlStatement);
                break;
            default:
                throw NotImplementedException("Cannot plan sqlStatement of type %s!", StatementTypeToString(sqlStatement->type));
        }
    }
