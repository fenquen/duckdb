...
namespace duckdb {
    ...
    BoundStatement Binder::Bind(TransactionStatement &transactionStatement) {
        // transaction statements do not require a valid transaction
        properties.requires_valid_transaction = transactionStatement.info->type == TransactionType::BEGIN_TRANSACTION;

        BoundStatement boundStatement;
        boundStatement.names = {"Success"};
        boundStatement.types = {LogicalType::BOOLEAN};
        boundStatement.plan = make_unique<LogicalSimple>(LogicalOperatorType::LOGICAL_TRANSACTION,
                                                         std::move(transactionStatement.info));
