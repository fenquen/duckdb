...
namespace duckdb {
    ...
    unique_ptr<BoundTableRef> Binder::Bind(BaseTableRef &baseTableRef) {
        QueryErrorContext error_context(root_statement, baseTableRef.query_location);
        // CTEs and views are also referred to using BaseTableRefs, hence need to distinguish here
        // check if the table name refers to a CTE
        auto cte = FindCTE(baseTableRef.table_name, baseTableRef.table_name == alias);
        if (cte) {
            // Check if there is a CTE binding in the BindContext
            auto ctebinding = bind_context.GetCTEBinding(baseTableRef.table_name);
            if (!ctebinding) {
                if (CTEIsAlreadyBound(cte)) {
                    throw BinderException("Circular reference to CTE \"%s\", use WITH RECURSIVE to use recursive CTEs", baseTableRef.table_name);
                }

                // Move CTE to subquery and bind recursively
                SubqueryRef subquery(unique_ptr_cast<SQLStatement, SelectStatement>(cte->query->Copy()));
                subquery.alias = baseTableRef.alias.empty() ? baseTableRef.table_name : baseTableRef.alias;
                subquery.column_name_alias = cte->aliases;
                for (idx_t i = 0; i < baseTableRef.column_name_alias.size(); i++) {
                    if (i < subquery.column_name_alias.size()) {
                        subquery.column_name_alias[i] = baseTableRef.column_name_alias[i];
                    } else {
                        subquery.column_name_alias.push_back(baseTableRef.column_name_alias[i]);
                    }
                }
                ...
                auto b = ctebinding;
                auto alias = baseTableRef.alias.empty() ? baseTableRef.table_name : baseTableRef.alias;
                auto names = BindContext::AliasColumnNames(alias, b->names, baseTableRef.column_name_alias);

                bind_context.AddGenericBinding(index, alias, names, b->types);
                // Update references to CTE
                auto cteref = bind_context.cte_references[baseTableRef.table_name];
                (*cteref)++;
        ...
        auto table_or_view =catalog.GetEntry(context,
                                             CatalogType::TABLE_ENTRY,
                                             baseTableRef.schema_name,
                                             baseTableRef.table_name,
                                             true,
                                             error_context);
        if (!table_or_view) {
            auto table_name = baseTableRef.schema_name.empty() ? baseTableRef.table_name : (baseTableRef.schema_name + "." + baseTableRef.table_name);
            // table could not be found: try to bind a replacement scan
            auto &config = DBConfig::GetConfig(context);
            for (auto &scan: config.replacement_scans) {
                auto replacement_function = scan.function(context, table_name, scan.data.get());
                if (replacement_function) {
                    replacement_function->alias = baseTableRef.alias.empty() ? baseTableRef.table_name : baseTableRef.alias;
                    replacement_function->column_name_alias = baseTableRef.column_name_alias;
                    return Bind(*replacement_function);
                }
            }
            // we still didn't find the table
            if (GetBindingMode() == BindingMode::EXTRACT_NAMES) {
                // if we are in EXTRACT_NAMES, we create a dummy table baseTableRef
                AddTableName(table_name);

                // add a bind context entry
                auto table_index = GenerateTableIndex();
                auto alias = baseTableRef.alias.empty() ? table_name : baseTableRef.alias;
            ...
            table_or_view = Catalog::GetCatalog(context).GetEntry(context, CatalogType::TABLE_ENTRY, baseTableRef.schema_name,
                                                                  baseTableRef.table_name, false, error_context);
        }
        ...
        switch (table_or_view->type) {
            case CatalogType::TABLE_ENTRY: {
                ...
                table_names = BindContext::AliasColumnNames(alias, table_names, baseTableRef.column_name_alias);

                auto logical_get = make_unique<LogicalGet>(table_index,
                                                           scan_function,
                                                           move(bind_data),
                                                           move(return_types),
                                                           move(return_names));

                bind_context.AddBaseTable(table_index,
                                          alias,
                                          table_names,
                                          table_types,
            ...
            case CatalogType::VIEW_ENTRY: {
                ...
                subquery.column_name_alias =
                        BindContext::AliasColumnNames(subquery.alias, view_catalog_entry->aliases, baseTableRef.column_name_alias);
