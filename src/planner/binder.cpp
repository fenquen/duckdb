...
namespace duckdb {
    ...
    BoundStatement Binder::Bind(SQLStatement &sqlStatement) {
        root_statement = &sqlStatement;
        switch (sqlStatement.type) {
            case StatementType::SELECT_STATEMENT:
                return Bind((SelectStatement &) sqlStatement);
            case StatementType::INSERT_STATEMENT:
                return Bind((InsertStatement &) sqlStatement);
            case StatementType::COPY_STATEMENT:
                return Bind((CopyStatement &) sqlStatement);
            case StatementType::DELETE_STATEMENT:
                return Bind((DeleteStatement &) sqlStatement);
            case StatementType::UPDATE_STATEMENT:
                return Bind((UpdateStatement &) sqlStatement);
            case StatementType::RELATION_STATEMENT:
                return Bind((RelationStatement &) sqlStatement);
            case StatementType::CREATE_STATEMENT:
                return Bind((CreateStatement &) sqlStatement);
            case StatementType::DROP_STATEMENT:
                return Bind((DropStatement &) sqlStatement);
            case StatementType::ALTER_STATEMENT:
                return Bind((AlterStatement &) sqlStatement);
            case StatementType::TRANSACTION_STATEMENT:
                return Bind((TransactionStatement &) sqlStatement);
            case StatementType::PRAGMA_STATEMENT:
                return Bind((PragmaStatement &) sqlStatement);
            case StatementType::EXPLAIN_STATEMENT:
                return Bind((ExplainStatement &) sqlStatement);
            case StatementType::VACUUM_STATEMENT:
                return Bind((VacuumStatement &) sqlStatement);
            case StatementType::SHOW_STATEMENT:
                return Bind((ShowStatement &) sqlStatement);
            case StatementType::CALL_STATEMENT:
                return Bind((CallStatement &) sqlStatement);
            case StatementType::EXPORT_STATEMENT:
                return Bind((ExportStatement &) sqlStatement);
            case StatementType::SET_STATEMENT:
                return Bind((SetStatement &) sqlStatement);
            case StatementType::LOAD_STATEMENT:
                return Bind((LoadStatement &) sqlStatement);
            case StatementType::EXTENSION_STATEMENT:
                return Bind((ExtensionStatement &) sqlStatement);
            case StatementType::PREPARE_STATEMENT:
                return Bind((PrepareStatement &) sqlStatement);
            case StatementType::EXECUTE_STATEMENT:
                return Bind((ExecuteStatement &) sqlStatement);
            case StatementType::LOGICAL_PLAN_STATEMENT:
                return Bind((LogicalPlanStatement &) sqlStatement);
            default:
                throw NotImplementedException("Unimplemented sqlStatement type \"%s\" for Bind",
                                              StatementTypeToString(sqlStatement.type));
        }
    }
    ...
    unique_ptr<BoundQueryNode> Binder::BindNode(QueryNode &queryNode) {
        // first we visit the set of CTEs and add them to the bind context
        AddCTEMap(queryNode.cte_map);
        // now we bind the queryNode
        unique_ptr<BoundQueryNode> boundQueryNode;
        switch (queryNode.type) {
            case QueryNodeType::SELECT_NODE:
                boundQueryNode = BindNode((SelectNode &) queryNode);
                break;
            case QueryNodeType::RECURSIVE_CTE_NODE:
                boundQueryNode = BindNode((RecursiveCTENode &) queryNode);
                break;
            default:
                D_ASSERT(queryNode.type == QueryNodeType::SET_OPERATION_NODE);
                boundQueryNode = BindNode((SetOperationNode &) queryNode);
                break;
        }
        return boundQueryNode;
    }

    BoundStatement Binder::Bind(QueryNode &queryNode) {
        auto boundQueryNode = BindNode(queryNode);

        BoundStatement result;
        result.names = boundQueryNode->names;
        result.types = boundQueryNode->types;

        // and plan it
        result.plan = CreatePlan(*boundQueryNode);
        return result;
    }
    ...
    BoundStatement Binder::BindReturning(vector<unique_ptr<ParsedExpression>> returning_list, TableCatalogEntry *table,
        ...
        properties.statementReturnType = StatementReturnType::QUERY_RESULT;
        return result;
    }
