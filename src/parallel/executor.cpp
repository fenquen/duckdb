...
namespace duckdb {
    ...
    struct ScheduleEventData {
        ScheduleEventData(const vector<shared_ptr<MetaPipeline>> &meta_pipelines,
                          ...
                          bool initial_schedule) : meta_pipelines(meta_pipelines),
                                                   events(events),
    ...
    void Executor::Initialize(PhysicalOperator *physicalPlan) {
        Reset();
        InitializeInternal(physicalPlan);
    }

    void Executor::InitializeInternal(PhysicalOperator *physicalPlan) {
        auto &taskScheduler = TaskScheduler::GetScheduler(context);

        {
            ...
            this->producer = taskScheduler.CreateProducer();

            // build and ready the pipelines
            PipelineBuildState pipelineBuildState;
            auto rootPipeline = make_shared<MetaPipeline>(*this, pipelineBuildState, nullptr);
            rootPipeline->Build(physical_plan);
            rootPipeline->Ready();
            ...
            rootPipeline->GetPipelines(root_pipelines, false);
            root_pipeline_idx = 0;

            // collect all meta-pipelines from the root pipeline
            vector<shared_ptr<MetaPipeline>> to_schedule;
            rootPipeline->GetMetaPipelines(to_schedule, true, true);
            ...
            rootPipeline->GetPipelines(pipelines, true);
